...
export default function TasksPage() {
  ...
  const handleUpdateTaskStatus = async (task, newStatus) => {
    try {
      const response = await fetch(`http://localhost:5000/api/employees/tasks/${task.id}`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      // If the task is marked as completed, show the rating modal
  ...
  const ratedTasks = tasks.filter(task => 
    task.status === 'Completed' && task.productivity_rating !== null
  );
    ...
    >
      ...
      <div>
        <div className="flex items-center justify-between mb-6">
          <h1 className="text-2xl font-semibold text-neutral-800">Active Tasks</h1>
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-neutral-200">
              ...
              <tbody className="divide-y divide-neutral-200">
                {filteredTasks.filter(task => !task.productivity_rating).length > 0 ? (
                  filteredTasks.filter(task => !task.productivity_rating).map((task) => (
                    <tr key={task.id} className="hover:bg-neutral-50">
                      ...
                      <td className="py-4 px-6">
                        {task.status === 'Completed' && !task.productivity_rating && (
                          <button
                  ...
                  <tr>
                    <td colSpan="6" className="py-4 px-6 text-center text-neutral-500">
      ...
      {/* Past Tasks Section */}
      <div>
        <h2 className="text-2xl font-semibold text-neutral-800 mb-6">Past Tasks</h2>
        <div className="bg-white rounded-lg shadow-card overflow-hidden">
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-neutral-200">
              <thead className="bg-neutral-50 border-y border-neutral-200">
                ...
                {ratedTasks.length > 0 ? (
                  ratedTasks.map((task) => (
                    <tr key={task.id} className="hover:bg-neutral-50">
                      ...
                      <td className="py-4 px-6">
                        <span className="text-sm text-neutral-600">{formatDate(task.rated_at)}</span>
                      </td>
                      <td className="py-4 px-6">
                        <div className="flex flex-col">
                          <span className="text-sm font-medium text-neutral-900">
...
const RatingBadge = ({ value }) => {
  ...
  return (
    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getBgColor(value)}`}>
      {value}/10
    </span>
  );
};